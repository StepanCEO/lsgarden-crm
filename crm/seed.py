...
KNOWLEDGE_ARTICLES_SEED = [
    {'role': 'front', 'title': 'Стандарт ответа по наличию', 'body': 'Сначала проверяем остаток в 1С, затем даем краткий ответ, предлагая альтернативу и время доставки.'},
    {'role': 'hybrid', 'title': 'Скрипт для отсутствующего товара', 'body': 'Если товара нет, уточняем срочность, предлагаем под заказ и ставим follow-up через 24 часа.'},
    {'role': 'content', 'title': 'Корпоративный КП', 'body': 'Используем шаблон с ассортиментом, сроками, условиями оплаты и фото-референсами.'},
    {'role': 'back', 'title': 'Приемка из 1С', 'body': 'После импорта проверяем расхождения по остаткам, себестоимости и листу производства.'},
    {'role': 'locomotive', 'title': 'Чат-бот выходного дня', 'body': 'Сообщает адрес, график и предлагает оставить контакт для обратной связи в рабочее время.'},
    {'role': 'admin', 'title': 'Режим "Быстрый тест"', 'body': 'Администратор может заблокировать экран для мини-теста по ассортименту или стандартам сервиса.'},
    {'role': 'front', 'title': 'Как приветствовать клиента', 'body': 'Здороваемся, представляемся и сразу задаем один уточняющий вопрос по запросу.'},
    {'role': 'front', 'title': 'Возражение по цене', 'body': 'Сравниваем не просто цену, а ценность: свежесть, подбор, упаковку и доставку.'},
    {'role': 'hybrid', 'title': 'Работа с жалобой', 'body': 'Не спорим с клиентом, фиксируем проблему и сразу предлагаем конкретный следующий шаг.'},
    {'role': 'back', 'title': 'Что делать при расхождении остатков', 'body': 'Проверяем справочник, сверку из 1С и историю изменений, затем пересчитываем остатки.'},
    {'role': 'content', 'title': 'Как оформлять КП', 'body': 'В КП должны быть состав, цена, сроки, доставка, условия оплаты и контакты ответственного.'},
    {'role': 'locomotive', 'title': 'Ответ в нерабочее время', 'body': 'Сообщаем график и просим оставить номер телефона для обратной связи.'},
]
...
def seed_demo_data():
    ...
    KnowledgeArticle.objects.bulk_create([
        KnowledgeArticle(**article) for article in KNOWLEDGE_ARTICLES_SEED
    ])
